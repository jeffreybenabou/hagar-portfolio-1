    ...
    <div className="absolute top-[20px] left-0 right-0 flex justify-between -translate-y-full">
      {arrows.slice(0, 2).map((arrow, index) => (
        <React.Fragment key={index}>
          <Arrow src={arrow} alt={`Arrow ${index + 1}`} className="hidden md:block"/>
  ...
  return (
    <>
      ...
      <Arrow
        src={arrow}
        alt="Arrow 3"
        className="absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-[200%] hidden md:block"
      />
    </>
  );
};
  ...
  return (
    <>
      <Arrow
        src={arrow}
        alt="Arrow 3"
        className="absolute left-1/2 top-1/2 -translate-y-1/2 translate-x-[100%] hidden md:block"
      />
      <PainPointsBox
